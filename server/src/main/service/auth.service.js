...
let signUp = async (data) => {
    if (!data.email || !data.password) {
        throw new ResourceNotFoundError("Missing details for signup");
    }
...
let signIn = async (data) => {
    if (!data.email || !data.password) {
        throw new ResourceNotFoundError("req.body missing some input attribute for sign_in create operation");
    }
    const existingUser = await userDao.findByEmailWithRole(data.email)
    if (!existingUser) throw new UserError.UserDataError("No user found")

    let passwordIsValid = bcrypt.compareSync(data.password, existingUser.password)

    if (!passwordIsValid) {
        throw new BadCredentialsError("Wrong username or password")
    }
